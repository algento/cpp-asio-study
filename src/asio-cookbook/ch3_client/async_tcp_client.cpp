...
class AsyncTCPClient : public asio::noncopyable {
 public:
    ...
    AsyncTCPClient() {
        ...
        m_thread = std::make_unique<std::thread>([this]() { m_ioc.run(); });
 ...
 private:
    ...
    /**
     * @brief 남은 비동기 연산이 없어도 이벤트 루프를 끝내지 않기 위해 필요하다.
     */
    std::unique_ptr<work_guard_type> m_work = nullptr;

    /**
     * @brief asio 이벤트 루프를 돌면서 비동기 연산을 수행할 콜백함수를 호출하는 I/O 스레드
     */
